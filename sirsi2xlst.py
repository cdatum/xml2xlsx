...
def convert_xml_to_xlsx(xml_file):    
    ...
    with open(xml_file, encoding = 'utf-8') as booklist:
        ...
        catalog = soup.find_all('catalog')
        
        tag = soup.marcEntry  
        
        #call = soup.find_all('totalCharges')
        #print(call)
        '''
        charges = []
        for item in catalog:            
            charges.append(item.totalCharges.get_text())           
            #print(item.totalCharges.get_text())
        '''
        ...
        for item in catalog:           
            ...
            # Get a count of the total checkouts            
            total_charges = 0
            for charges in item.find_all('totalCharges'):
                checkout = charges.get_text()
                checkout = int(checkout)
                total_charges += checkout
            
            # Get a list of all of the barcodes            
            barcodes = []
            for codes in item.find_all('itemID'):
                barcodes.append(codes.get_text())          
            ...
            item_details.extend([title,bcodes,campus,call_no,isbn,desc,total_charges,date_last_use])
            
            item_list.append(item_details)
            
            # Collect some basic data for item stats 
            item_stats = []
            elements = (bcodes, stats_title, total_charges)
            ...
            #totalChargesprint("Charges: " + str(count) + " " + title )       
